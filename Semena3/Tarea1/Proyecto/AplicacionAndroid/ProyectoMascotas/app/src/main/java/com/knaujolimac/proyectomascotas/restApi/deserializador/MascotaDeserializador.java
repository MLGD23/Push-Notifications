...
public class MascotaDeserializador implements JsonDeserializer<MascotaResponse> {
    ...
    public MascotaResponse deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        ...
        switch (opcionDeserealiar){
            case ConstantesRestApi.DESEREALIZAR_MEDIA_RECENT:
                mascotas = this.deserealizarMediaRecent(mascotaResponseData);
                break;
            case ConstantesRestApi.DESEREALIZAR_USER_SEARCH:
    ...
    public ArrayList<MascotaPerfil>  deserealizarMediaRecent(JsonArray mascotaResponseData ){
        ...
        for (int i = 0; i < mascotaResponseData.size() ; i++) {
            ...
            JsonObject mediaResponseDataObject = mascotaResponseData.get(i).getAsJsonObject();

            String idFoto = mediaResponseDataObject.get(JsonKeys.MEDIA_ID).getAsString();

            JsonObject userJson     = mediaResponseDataObject.getAsJsonObject(JsonKeys.USER);
